...
class AgentPlanner:
    ...
    def _meta(self, channel_id: str) -> Dict[str, Any]:
        ...
        try:
            if hasattr(meta, "keys"): return {k: meta[k] for k in meta.keys()}
        except Exception: pass
        return {}

    def _is_bad_summary(self, text: str) -> bool:
        low = (text or "").lower()
        bad_fragments = ["не могу посмотреть", "не может посмотреть", "не могу прочитать", "не может прочитать", "не могу видеть", "не может видеть", "cannot read", "cannot view", "не знаю что там было"]
        return any(frag in low for frag in bad_fragments)

    def _fallback_summary_from_recent(self, recent) -> str:
        lines = []
        for row in recent[-5:]:
            role, content = (row.get("role") or "").strip().lower(), (row.get("content") or "").strip()
    ...
    def _trim_recent_for_current_user(self, recent, user_text: str):
        ...
        for v in variants:
            if normalize_compare_text(v) == norm: return recent[:-1]
        return recent

    def _last_assistant_reply(self, recent) -> str:
        for row in reversed(recent or []):
            if (row.get("role") or "").lower() == "assistant": return (row.get("content") or "").strip()
        return ""
    ...
    def _looks_like_echo(self, candidate: str, user_text: str, recent) -> bool:
        ...
        if not low or low in {"мм", "мм?", "м?", "ок", "ok", "поняла"}: return True
        if len(low) < 2: return True
        if is_too_similar(candidate, user_text, threshold=0.86): return True
        for row in (recent or []):
            if is_too_similar(candidate, row.get("content") or "", threshold=0.92): return True
        last_assistant = self._last_assistant_reply(recent)
        if last_assistant and is_too_similar(candidate, last_assistant, threshold=0.92): return True
        return False

    def _read_followup(self, low: str) -> bool:
        return any(p in low for p in ["а еще", "а ещё", "еще", "дальше", "продолжай", "подробнее", "сводку", "что обсуждали", "continue", "more", "what else", "before that", "earlier", "кто начал", "после этого", "до конфликта"])

    def _read_limit_from_text(self, low: str) -> int:
        if any(p in low for p in ["подроб", "полностью", "весь", "вся", "развернуто"]): return 80
        if any(p in low for p in ["сводка", "что обсуждали", "что происходило"]): return 50
        if any(p in low for p in ["последнее", "last", "latest"]): return 1
        return 30
    ...
    async def refresh_user_card(self, message: discord.Message, user_text: str) -> None:
        ...
        if not (seen < 3 or seen % 8 == 0): return
        ...
        payload = f"Текущая карточка:\n{current_profile or 'нет'}\n\nПоследние сообщения пользователя:\n" + "\n".join(f"- {x}" for x in user_lines) + f"\n\nКонтекст канала:\n" + "\n".join(f"- {x}" for x in context_lines)
        try:
            raw = await self.llm.chat_json([{"role": "system", "content": USER_CARD_SYSTEM_PROMPT}, {"role": "user", "content": payload}], temperature=0.2, max_tokens=400)
        except Exception: return
        if not isinstance(raw, dict) or not raw: return

        updates: Dict[str, Any] = {"username": message.author.name, "display_name": message.author.display_name, "messages_seen": seen}
        fields = ["summary", "personality_traits", "humor_style", "toxicity_level", "friendliness", "relationship", "relationship_trend", "recurring_topics", "inside_jokes", "nicknames", "bot_opinion", "confidence_score"]
        for f in fields:
            val = raw.get(f)
    ...
    def update_affinity_from_message(self, author_id: str, user_text: str):
        low = user_text.lower()
        delta = 1 if any(w in low for w in POSITIVE_WORDS) else -1 if any(w in low for w in NEGATIVE_WORDS) else 0
    ...
    def build_snapshot(self, message: discord.Message, user_text: str) -> str:
        ...
        meta = self._meta(cid)
        user_card = self.store.build_profile_card(subject_id=str(message.author.id), subject_name=message.author.display_name, max_facts=6)
        lines = ["КОНТЕКСТ ДЛЯ Nika:", f"Настроение: {meta.get('mood', 'calm')}, Энергия: {float(meta.get('energy', 0.5)):.2f}, Sass: {float(meta.get('sass', 0.5)):.2f}", f"Эмоция: {self.detect_emotion(user_text)}", f"ПАМЯТЬ ОБ АВТОРЕ:\n{user_card}"]
    ...
    def build_chat_messages(self, message: discord.Message, user_text: str):
        ...
        msgs.append({"role": "user", "content": f"{message.author.display_name}: {user_text}"})
        return msgs
    ...
    def _action_from_llm(self, obj: dict) -> dict:
        ...
        if action == "remember": return {"action": "remember", "key": "note", "value": (obj.get("value") or obj.get("text") or "").strip()}
        if action == "read_channel": return {"action": "read_channel", "channel": (obj.get("channel") or "").strip(), "limit": int(obj.get("limit") or 30), "before": (obj.get("before") or "").strip()}
        if action == "send_message": return {"action": "send_message", "channel": (obj.get("channel") or "").strip(), "text": (obj.get("text") or "").strip()}
        if action == "ping_user": return {"action": "ping_user", "channel": (obj.get("channel") or "").strip(), "user": (obj.get("user") or "").strip(), "text": (obj.get("text") or "").strip()}
        if action == "react": return {"action": "react", "channel": (obj.get("channel") or "").strip(), "reaction": (obj.get("reaction") or "").strip()}
    ...
    async def decide(self, message: discord.Message, user_text: str) -> Dict[str, Any]:
        ...
        if direct:
            if direct.get("action") == "reply" and any(k in low for k in ["не сюда", "в другой", "перепутал"]): return {"action": "ignore"}
            return direct
        meta = self._meta(cid)
        if self._read_followup(low) and meta.get("last_action_type") == "read_channel":
            ...
            if any(k in low for k in ["сводк", "коротк", "обсуждали"]) and meta.get("last_read_summary") and not structured: return {"action": "reply", "text": meta["last_read_summary"]}
            return {"action": "read_channel", "channel": meta.get("last_target_channel_id"), "limit": 80 if structured else self._read_limit_from_text(low), "before": meta.get("last_read_first_message_id") if any(k in low for k in ["до этого", "раньше", "before", "earlier"]) else ""}
        for attempt in range(2):
            msgs = self.build_chat_messages(message, user_text)
            raw = await self.llm.chat(msgs, temperature=0.3 + (attempt * 0.1), frequency_penalty=0.5, presence_penalty=0.3)
            if not raw: continue
            parsed = None
    ...
    async def summarize_channel(self, channel_id: str, force: bool = False):
        ...
        if not force and last_ts > 0 and (now - last_ts < 1200) and (int(meta.get("message_count") or 0) - int(meta.get("last_summary_count") or 0) < 10): return
        recent = self.store.get_recent_history(channel_id, 50)
        recent_text = "\n".join(f"{m['role']}: {m['content']}" for m in recent)
        if not recent_text.strip(): return
        try: raw = await self.llm.chat_json([{"role": "system", "content": SUMMARY_SYSTEM_PROMPT}, {"role": "user", "content": f"История:\n{recent_text}"}], temperature=0.2, max_tokens=500)
        except Exception: return
        ...
        self.store.update_channel_meta(channel_id, summary=brief[:self.settings.max_summary_chars], summary_timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat(), last_summary_count=int(meta.get("message_count") or 0), summary_participants=", ".join(raw.get("participants") or []), summary_topics=", ".join(raw.get("topics") or []), summary_mood=str(raw.get("mood") or ""), summary_jokes=", ".join(raw.get("jokes") or []), summary_conflicts=", ".join(raw.get("conflicts") or []), summary_events=", ".join(raw.get("events") or []), summary_unresolved=", ".join(raw.get("unresolved") or []))
    ...
    def maybe_autonomy_prompt(self, message: discord.Message):
        ...
        now = time.time()
        if now - self._parse_db_timestamp(meta.get("last_autonomy_at")) < self.settings.autonomy_global_cooldown: return None
        ...
        payload = [{"role": "system", "content": AUTONOMY_SYSTEM_PROMPT}, {"role": "user", "content": f"Последние сообщения:\n{recent_text}\n\nУчастники:\n" + ("\n".join(f"- {p}" for p in participants) if participants else "- нет") + f"\n\nCooldowns: interjection={int(now - last_int)}s, emoji={int(now - last_emo)}s (min {self.settings.autonomy_interjection_cooldown}s and {self.settings.autonomy_emoji_cooldown}s)"}]
        return payload, last_int, last_emo
    ...
    async def run_autonomy(self, message: discord.Message) -> Dict[str, Any]:
        ...
        prompt, last_int, last_emo = res
        now = time.time()
        try: raw = await self.llm.chat_json(prompt, temperature=0.4, max_tokens=250)
        ...
        if action in {"short_interject", "contextual_reply", "reply", "sarcastic_comment", "playful_question", "meme_reply"}:
            if now - last_int < self.settings.autonomy_interjection_cooldown: return {"action": "ignore"}
            text = (raw.get("text") or raw.get("message") or "").strip()
            if text and not self._looks_like_echo(text, "", self.store.get_recent_history(str(message.channel.id), 5)): return {"action": action, "text": text}
        elif action == "react":
            if now - last_emo < self.settings.autonomy_emoji_cooldown: return {"action": "ignore"}
